if (document.querySelector('#play-pause-button')) {
  ...
  $('#recording-exposed').click(function () {
    if ($(this).is(':hidden')) {
      let confirmText = 'Are you sure that you want to make this recording VISIBLE by the user?'
    } else {
      let confirmText = 'Are you sure that you want to make this recording HIDDEN from the user?'
    }
    ...
    if (confirmed) {
      $.post('/set_user_can_access', { id: $('#video-element').data('recording-id'), make_visible: confirmed}, function (json) {
        ...
        console.log('Error setting recording visibility')
      });
    }
})
