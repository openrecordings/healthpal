class PlayController < ApplicationController
  def index
    ...
    @users = if current_user.privileged? && current_user.email != 'ma.admin@audiohealthpal.com'
               User.joins(:recordings).order(:email).uniq
             else
               # All users who are currently sharing with current_user
               [current_user] + Share.shared_with_user(current_user).map { |s| s.user }
  ...
  def play
    ...
    if @recording && current_user.can_access(@recording)
      ...
      @provider = UserField.find_by(recording: @recording,
                                    type: :provider) || UserField.new(
                                      recording: @recording, type: :provider
                                    )
      @note = UserField.find_by(recording: @recording,
                                type: :note) || UserField.new(
                                  recording: @recording, type: :note
                                )
  ...
  def prepare_segments(recording)
    return_segments = []
    multi_segment = nil
    segments = recording.transcript_segments.includes(:annotations)
    segments.each do |segment|
      next unless segment.annotations.any?
      ...
      elsif segment.tmp_annotation_categories == multi_segment.tmp_annotation_categories
  ...
  def grouped_annotations(recording)
    ...
    TagType.all.order(:label).each do |tag_type|
      annotations_for_type = annotations.select do |a|
                               a.category == tag_type.label
                             end.uniq { |a| a.text.titleize }.sort_by { |a| a.text.titleize }
    ...
    groups_with_annotations = groups_with_annotations.sort_by do |tag_type_hash|
      tag_type_hash.keys.first.label
    end
    groups_without_annotations = groups_without_annotations.sort_by do |tag_type_hash|
      tag_type_hash.keys.first.label
    end
